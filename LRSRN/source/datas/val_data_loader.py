...
class VALPhaseLoader(data.Dataset):
    def __init__(
        ...
        for idx in range(self.start_idx, self.end_idx):
            #idx = str(i).zfill(4)
            hr_filename = os.path.join(self.HR_folder, str(idx) + self.img_postfix)
            #lr_filename = os.path.join(self.LR_folder, 'X{}'.format(self.scale), idx + 'x{}'.format(self.scale) + self.img_postfix)
            lr_filename = os.path.join(self.LR_folder, str(idx) + '.jpg')
        ...
        hr_dir = os.path.join(self.cache_dir, 'valphase_hr', 'ycbcr' if self.colors==1 else 'rgb')
        lr_dir = os.path.join(self.cache_dir, 'valphase_lr_x{}'.format(self.scale), 'ycbcr' if self.colors==1 else 'rgb')
...
if __name__ == '__main__':
    ...
    argment   = True
    div2k = VALPhaseLoader(HR_folder, LR_folder, augment=True, scale=2, colors=3, patch_size=96, repeat=168, store_in_ram=True)
